...
class SLayerSquare(Module):
    def __init__(self, n_elements: int,
                 point_dimension: int = 2,
                 multi_init: Tensor = None,
                 additive_init: Tensor = None):
        """
                Args:
                    n_elements:
        ...
        additive_init = parameter_init_from_arg(
            additive_init,
            expected_init_size,
            lambda size: torch.ones(*size) * 3)

        self.multi = Parameter(multi_init)
        self.additive = Parameter(additive_init)
    ...
    def forward(self, input)->Tensor:
        ...
        x = batch
        x= torch.square(x[:,:,1]- x[:,:,0])
        #print("x.shape[1] for CC or H1", x)

        x= torch.nn.Linear(x.shape[1], self.n_elements, device= x.device)(x)
        ##x= x*x.shape[1]*x.shape[1]
        #x= x*not_dummy_points.shape[0]*not_dummy_points.shape[0]
        # if x.shape[1]==self.n_elements:
        #     print("x if x.shape[1]==self.n_elements: ", x)
        # else:
        #     print("x otherwise: ", x)
        return x
...
class SLayerRationalHat(Module):
    ...
    def forward(self, input)->Tensor:
        ...
        return x, self.centers, self.radius
