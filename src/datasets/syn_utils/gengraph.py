...
#
###################################
def gen_ba(nb_shapes=80, width_basis=300, feature_generator=None, m=5, is_weighted=False):
    """Synthetic Graph #1:
    ...
    if is_weighted:
        G, role_id = synthetic_structsim.ba(0, width_basis, m=m)
    else: 
        G, role_id = synthetic_structsim.ba(0, width_basis, m=m)
...
def gen_ba_grid_not_cycle(nb_shapes=80, width_basis=300, feature_generator=None, m=5, is_weighted=False):
    basis_type = "ba"
    # [shape_name, weight for edges, other args]
    list_shapes = [["grid", 1, 3], ["cycle", 0, 5]]

    if is_weighted:
        G, role_id, _ = synthetic_structsim.build_mixed_weighted_graph(
            width_basis, basis_type, list_shapes, start=0, m=m
        )
    else: 
        G, role_id, _ = synthetic_structsim.build_graph(
...
def gen_ba_house_not_cycle(nb_shapes=80, width_basis=300, feature_generator=None, m=5, is_weighted=False):
    basis_type = "ba"
    list_shapes = [["house", 1], ["cycle", 0, 5]]
...
def gen_ba_house_grid(nb_shapes=80, width_basis=300, feature_generator=None, m=5, is_weighted=False):
    basis_type = "ba"
    list_shapes = [["house"], ["grid", 3]]
...
def gen_ba_bottle(nb_shapes=80, width_basis=300, feature_generator=None, m=5, is_weighted=True):
    """Synthetic Graph #6:
...
def gen_ba_cycle(nb_shapes=80, width_basis=300, feature_generator=None, m=5, is_weighted=True):
    basis_type = "ba"
