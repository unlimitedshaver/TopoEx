...
from .cell_complex import Cochain, Complex   ###
...
def pyg_to_simplex_tree(edge_index: Tensor, size: int, edge_label=None):
    ...
    for e in range(edges.shape[1]):
        ...
        if edge_label is not None:
            st.insert(edge, filtration=edge_label[e])
        else:
            st.insert(edge)     ##Deprecated since version 3.2.0. insert方法在新的版本中已经弃用，可以尝试使用insert_batch
...
def build_tables(simplex_tree, size):
    ...
    for simplex, label in simplex_tree.get_simplices():  ##这里的label默认是filtration=0
        dim = len(simplex) - 1
        if dim == 0:
            continue
    ...
    return simplex_tables, id_maps, edge_label
...
def construct_features(vx: Tensor, cell_tables, init_method: str) -> List:
    ...
    for dim in range(1, len(cell_tables)):
        ...
        in_features = vx.index_select(0, node_cell_index[0])    ##在0维度上，node_cell_index[0]索引上选择元素，该索引是一维张量LongTensor
        features.append(scatter(in_features, node_cell_index[1], dim=0, ##将源散布到目标，dim=0意味着对src维度0进行操作获得第index个元素
                                dim_size=len(cell_tables[dim]), reduce=init_method))

    return features


def extract_labels(y, size, node_label=None, edge_label=None):
    v_y, e_y, complex_y = node_label, edge_label, None
    if y is None:
        return v_y, e_y, complex_y
...
def get_rings(edge_index, max_k=7):
    ...
    graph_gt = graph_tool.Graph(directed=False)
    graph_gt.add_edge_list(edge_list)
    graph_tool.stats.remove_self_loops(graph_gt)
    graph_tool.stats.remove_parallel_edges(graph_gt)
    ...
    for k in range(3, max_k+1):
        ...
        pattern_gt = graph_tool.Graph(directed=False)
...
def build_tables_with_rings(edge_index, simplex_tree, size, max_k):
    ...
    cell_tables, id_maps, edge_label = build_tables(simplex_tree, size)
    ...
    if len(rings) > 0:
        ...
        for cell in rings:
            next_id = len(cell_tables[2])       ##从0开始
            id_maps[2][cell] = next_id
            cell_tables[2].append(list(cell))

    return cell_tables, id_maps, edge_label
...
def compute_ring_2complex(x: Union[Tensor, np.ndarray], edge_index: Union[Tensor, np.ndarray],
                          ...
                          init_edges=True, init_rings=False, node_label=None, edge_label=None,
                          **kwargs) -> Complex:
    ...
    if isinstance(y, np.ndarray):
        y = torch.tensor(y)
    if isinstance(node_label, np.ndarray):
    ...
    simplex_tree = pyg_to_simplex_tree(edge_index, size, edge_label=edge_label)
    assert simplex_tree.dimension() <= 1
    if simplex_tree.dimension() == 0:
        assert edge_index.size(1) == 0

    # Builds tables of the cellular complexes at each level and their IDs
    cell_tables, id_maps, e_label = build_tables_with_rings(edge_index, simplex_tree, size, max_k)
    assert len(id_maps) <= 3
    ...
    # Construct features for the higher dimensions
    xs = [x, None, None]
    constructed_features = construct_features(x, cell_tables, init_method)  ##cell_tables是各个list(cell)的集合
    if simplex_tree.dimension() == 0:
    ...
    # Initialise the node / edge /  complex labels
    v_y, e_y, complex_y = extract_labels(y, size, node_label, torch.tensor(e_label))


    cochains = []
    for i in range(complex_dim + 1):
        y = {0: v_y, 1: e_y}.get(i, None)
        cochain = generate_cochain(i, xs[i], upper_idx, lower_idx, shared_boundaries, shared_coboundaries,
                               cell_tables, boundaries_tables, complex_dim=complex_dim, y=y)
        cochains.append(cochain)

    if node_type is not None:
        return Complex(*cochains, y=complex_y, dimension=complex_dim, node_type=node_type)
...
def convert_graph_dataset_with_rings(dataset, max_ring_size=7, include_down_adj=False,
                                     ...
                                     n_jobs=1, v_label=False, e_label=False, **kwargs):
    dimension = -1
    num_features = [None, None, None]

    ##额外
    # node_type for mutag
    nd_type = kwargs.get('nd_type', False)
    ...
    complexes = parallel(delayed(compute_ring_2complex)(
        ...
        node_label=maybe_convert_to_numpy(data.node_label) if v_label else None,
        edge_label=maybe_convert_to_numpy(data.edge_label) if e_label else None,
        node_type=maybe_convert_to_numpy(data.node_type) if nd_type else None) for data in dataset)
